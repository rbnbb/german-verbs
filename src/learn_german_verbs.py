...
def select_pronoun(conjugation : str) -> str:
    ...
    if person == "singular" or person == "plural":
...
def load_verbs() -> list:
    with open("data/verb_dict.pyobj", "rb") as file:
...
def interactive_loop(verbs : list):
    ...
    while True:
        ...
        rand_conjugation = random.choice([key for key in rand_entry.keys() if key != 'verb'])
